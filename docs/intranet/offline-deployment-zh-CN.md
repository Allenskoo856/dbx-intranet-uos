# DBX UOS 1070 / WebKitGTK 4.0 内网离线部署手册

本手册对应独立兼容分支 `agent/uos1070-webkit40`。它不替换 WebKitGTK 4.1 主线；该分支将 Wry、Tauri Linux runtime、WebKitGTK/JSC Rust binding 和 libsoup2 binding 固定在仓库内维护，目标是 UOS 1070 的 WebKitGTK 4.0/libsoup2 ABI。
...
本分支的 Tauri 2 兼容层使用 WebKitGTK 4.0 和 libsoup2，因此包声明以下运行时依赖：
...
“Debian 10-like”在这里表示 UOS 的 Debian-like 用户空间和兼容的软件包仓库，不代表原版 Debian 10 stock 用户空间可以直接运行。本分支最低按 WebKitGTK API 2.36、libsoup 2.62 进行构建探测；目标机仍须由发行版管理员确认对应的 `libwebkit2gtk-4.0-37`、`libjavascriptcoregtk-4.0-18` 和 `libsoup2.4` 运行库。当前 Release 未宣称已经在实体 UOS 机器上完成验收。
...
| Cargo registry/git 与 `target/` | `Swatinem/rust-cache@v2`，共享 key `uos1070-webkit40-offline-deb-x86_64-unknown-linux-gnu` | `Cargo.lock`、Rust 环境或共享 key 变化 | 复用 Cargo 依赖和已生成的 target 内容 |
...
构建前的 `Report build cache configuration` 和构建后的 `Report sccache statistics` 会记录 pnpm store、Cargo home、target 路径以及 sccache 命中统计。第一次变更缓存配置或依赖锁文件后的构建仍然是冷启动；后续本分支 push 或 `uos1070-v*` tag 可以复用。当前最慢步骤已定位为 Tauri/Rust 的 `Build UOS offline Debian package`，此前无 Rust 缓存实测约 21 分钟；pnpm 安装约数秒、Linux 系统依赖安装约 1 分钟，不是主要瓶颈。
...
```text
dist/uos1070-webkit40/
├── DBX_<version>_uos1070-webkit40_amd64.deb
├── DBX_<version>_uos1070-webkit40_amd64.deb.sha256
├── SHA256SUMS
└── manifest.json
```

GitHub Actions 工作流 `.github/workflows/uos-offline-deb.yml` 在本分支 push 和 `uos1070-v*` tag 上构建；tag 构建会把这些文件上传到 GitHub Release。Actions 是构建区，不是目标内网运行时；Release 下载后必须通过企业批准的介质传输到内网。
...
如果 `dpkg` 报缺少 `libwebkit2gtk-4.0-37`、`libjavascriptcoregtk-4.0-18` 或 libsoup2 等依赖，不要在隔离目标机上直接执行 `apt -f install`，因为它可能尝试访问外部软件源。应由管理员在有网构建区或批准的内网 apt 镜像中准备同架构依赖包，先通过受控介质校验并安装依赖，再重新执行 `dpkg -i`。目标机没有经过现场检查前，不能把依赖名等同于已经存在于某个 UOS 镜像。
...
```bash
sudo dpkg -i DBX_<new-version>_uos1070-webkit40_amd64.deb
```
